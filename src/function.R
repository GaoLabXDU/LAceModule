...
gene_correlation=function(geneset,rna.exp)
{
  ...
  open(matlab)
  setVariable(matlab,x=t(exp))
  evaluate(matlab,'[pcor,pvalue]=corr(x,x);')
  pcor=getVariable(matlab,'pcor')$pcor
  pvalue=getVariable(matlab,'pvalue')$pvalue
  close(matlab)
  
  rownames(pcor)=rownames(exp)
  colnames(pcor)=rownames(exp)
  rownames(pvalue)=rownames(exp)
  colnames(pvalue)=rownames(exp)
  pcor=pcor[geneset,geneset]
  pvalue=pvalue[geneset,geneset]
  
  result=list(correlation=pcor,pvalue=pvalue)
  return(result)
}
